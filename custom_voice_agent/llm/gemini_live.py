...
class GeminiLiveSession:
    ...
    async def connect(self):
        ...
        if config.api_key:
            ...
            logger.info(f"Connecting to Gemini Live via Google AI API (model={config.model})")
            ...
            logger.info(
                f"Connecting to Gemini Live via Vertex AI "
                f"(model={config.model}, project={config.project_id})"
            )
        else:
            raise ValueError("Either api_key or credentials must be provided")

        # Build LiveConnect configuration
    ...
    async def send_text(self, text: str, role: str = "user"):
        ...
        try:
            ...
            logger.exception("Error sending text to Gemini")
    ...
    async def send_activity_start(self):
        ...
        try:
            ...
            logger.debug("Error sending activity_start", exc_info=True)
    ...
    async def send_activity_end(self):
        ...
        try:
            ...
            logger.debug("Error sending activity_end", exc_info=True)
